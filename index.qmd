...
# Assuming your data is in a data frame called IPGTT_data
# IPGTT_data <- read.csv("your_data_file.csv")  # Uncomment and modify this line to load your data

# Step 2: Remove rows with missing values
IPGTT_data_clean <- IPGTT_data %>% filter(test_week == 1) %>% na.omit() 

# Convert time_point to a factor
...
# Step 4: Calculate the estimated marginal means for each genotype
emm <- emmeans(IPGTT_mixed_model, ~  genotype| time_point)

broom.mixed::tidy(emm) 

# Step 5: Calculate contrasts between genotype pairs for all test weeks
contrast_results <- pairs(emm, interaction = "pairwise")

# Convert contrast results to a data frame
contrast_df <- as.data.frame(contrast_results)

# Filter for week 1 in the contrasts
#contrast_df_week1 <- contrast_df %>% filter(test_week == 1)


# Step 7: Plot the contrasts for week 1
